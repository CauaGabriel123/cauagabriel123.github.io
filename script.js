// =========================
// LS STORE v11.1 — Cauã (6 correções aplicadas)
// =========================
...
function chip(container, value, onPick){
  const b=document.createElement('button'); b.textContent=value; 
  b.onclick=()=>{ [...container.children].forEach(x=>x.classList.remove('active')); b.classList.add('active'); onPick(value); };
...
function openModal(id){
  ...
  // NÃO seleciona nada por padrão — obrigatório escolher
  (p.sizes&&p.sizes.length?p.sizes:['Único']).forEach(v=>chip(sizeOptions,v,(val)=>{ chosenSize=val; checkEnableAdd(); }));
  (p.colors&&p.colors.length?p.colors:['Única']).forEach(v=>chip(colorOptions,v,(val)=>{ chosenColor=val; checkEnableAdd(); }));

  // botão começa desabilitado se houver opções
  modalAdd.disabled = (p.sizes && p.sizes.length>0) || (p.colors && p.colors.length>0);
...
function checkEnableAdd(){
  ...
  modalAdd.disabled = (needSize && !chosenSize) || (needColor && !chosenColor);
}
...
function addToCartFromModal(){
  ...
  // === (4) Tamanho/Cor obrigatórios ===
  const needSize = p.sizes && p.sizes.length>0;
  const needColor = p.colors && p.colors.length>0;
  if(needSize && !chosenSize){ alert('Escolha um tamanho.'); return; }
  if(needColor && !chosenColor){ alert('Escolha uma cor.'); return; }

  const key=p.id+'|'+(chosenSize||'Único')+'|'+(chosenColor||'Única');
  const ex=cart.find(i=>i.key===key);
  const finalPrice = p.discount?(p.price*(1-p.discount)):p.price;
  if(ex) ex.qty++; else cart.push({key,id:p.id,name:p.name,price:finalPrice,qty:1,size:chosenSize||'Único',color:chosenColor||'Única',img:p.img});
  renderCart();
...
document.getElementById('checkout').onclick=()=>{
  ...
  // === (6) Mensagem WhatsApp com pagamento + troco
  ...
  if (payment === 'Dinheiro') {
    ...
    if(chosen && chosen.value==='sim'){
      const v=parseFloat((document.getElementById('cash-amount').value||'').replace(',','.'));
      const total=calcTotals(); const change=(v-total);
      msg+=`%0A💵 *Valor pago:* R$ ${v.toFixed(2)}%0A🔁 *Troco:* R$ ${change.toFixed(2)}`;
    } else if (chosen && chosen.value==='nao'){
      msg+=`%0A💵 *Troco:* Não precisa`;
    }
  }

  msg+=`%0A📦 *Entrega:* ${delivery}`;
  if(address) msg+=`%0A🏡 *Endereço:* ${address} ${feeNote}`;
  ...
  // === (5) Popup antes do WhatsApp
  playChime();
  const pop=document.getElementById('popup-overlay');

  // mostrar popup robustamente (funciona com ou sem CSS atualizado)
  if (pop) {
    pop.hidden = false;
    try { pop.classList.add('show'); } catch(e){}
  ...
  setTimeout(()=>{
    ...
    // abre WhatsApp após fechar popup
...
function renderAccount(){
  ...
  if(!user){
    // === (1) Login mais moderno + (3) Botão Fechar
    accountArea.innerHTML = `
      <div class="auth-card">
        <div class="auth-title">
          <h3>Entrar</h3>
          <button class="close-auth" id="close-auth" title="Fechar">✕</button>
        </div>
        <label>E-mail<input id="login-email" type="email" placeholder="voce@email.com"></label>
        <label>Senha<input id="login-pass" type="password" placeholder="••••••••"></label>
        <div class="auth-actions">
          <button id="do-login" class="add-btn">Entrar</button>
        </div>
        <hr style="margin:14px 0;border:none;border-top:1px solid #eee">
        <div class="auth-title"><h3>Criar conta</h3></div>
        <label>Nome<input id="reg-name" type="text" placeholder="Seu nome"></label>
        <label>E-mail<input id="reg-email" type="email" placeholder="voce@email.com"></label>
        <label>Senha<input id="reg-pass" type="password" placeholder="Crie uma senha"></label>
        <div class="auth-actions">
          <button id="do-register" class="add-btn">Cadastrar</button>
        </div>
    ...
    if (closeBtn) closeBtn.onclick=()=>{ showSection('inicio'); };
...
// --- Atalho ESC para sair da conta (3)
document.addEventListener('keydown', (e)=>{
  const accountVisible = document.getElementById('minha-conta').classList.contains('visible');
  if(accountVisible && e.key === 'Escape'){ showSection('inicio'); }
});
