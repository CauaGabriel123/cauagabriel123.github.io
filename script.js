// LS STORE v11.1 — funções, carrinho, WhatsApp e popup corrigido
...
function playChime(){
  ...
  g.gain.setValueAtTime(.001,t); g.gain.exponentialRampToValueAtTime(.25,t+0.03);
...
function clickSoft(){
  ...
  g.gain.setValueAtTime(.001,t);g.gain.exponentialRampToValueAtTime(.1,t+0.02);
...
function animateToCart(fromImg){
  ...
  clone.className='fly';
  clone.style.cssText=`position:fixed;left:${r.left}px;top:${r.top}px;width:${r.width}px;height:${r.height}px;z-index:1000;transition:all .6s ease;`;
  document.body.appendChild(clone);
  setTimeout(()=>{clone.style.left=c.left+'px';clone.style.top=c.top+'px';clone.style.opacity='0';clone.style.transform='scale(0.2)';},10);
